...
import { user_delegate, pokemon_delegate } from "./models-delegate";
...
export async function fetchMany<T extends {id: string}>(
  model: {
    findMany:()=>Promise<T[]>
...
export async function fetchOne<T extends {id: string}>(
  model: {findUnique: (
    args: {
      where: {
        id: any
...
export async function deleteMany<T extends{id: string}>(
  model: {
